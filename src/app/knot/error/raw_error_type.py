...
class KnotErrorType(Enum):
	...
	# Transaction errors
	KNOT_TXN_EEXISTS = "too many transactions" # Открыты несколько транзакций одного класса
	KNOT_TXN_ENOTEXISTS = "no active transaction" # В данный момент не открыта ни одна транзакция
	KNOT_TXN_ETHREAD = "transaction thread mismatch" # (???) Ошибка потоков
